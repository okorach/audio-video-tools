...
class MediaFile:
    def __init__(self, filename):
        if not mediatools.utilities.is_media_file(filename):
            raise FileTypeError('File %s is neither video, audio nor image file' % filename)
        self.type = mediatools.utilities.get_file_type(filename)
...
class AudioFile(MediaFile):
    ...
    def get_tags(self):
        ...
        if mediatools.utilities.get_file_extension(self.filename).lower() is not 'mp3':
...
def get_media_properties(props_file = None):
    ...
    try:
        properties = mediatools.utilities.get_properties(props_file)
...
def build_target_file(source_file, profile, properties):
    ...
    if extension is None:
        extension = mediatools.utilities.get_file_extension(source_file)
    return mediatools.utilities.add_postfix(source_file, profile, extension)
...
def encode(source_file, target_file, profile, **kwargs):
    ...
    mediatools.utilities.debug(2, ffmpeg.get_args(stream))
    mediatools.utilities.debug(1, "%s --> %s" % (source_file, target_file))
    try:
...
def encode_album_art(source_file, album_art_file, **kwargs):
    ...
    properties = get_media_properties()
    target_file = mediatools.utilities.add_postfix(source_file, 'album_art')
...
def rescale(image_file, width, height, out_file = None):
    properties = get_media_properties()
    if out_file is None:
        out_file = mediatools.utilities.add_postfix(image_file, "%dx%d" % (width,height))
...
def deshake(video_file, width, height, out_file = None):
    ...
    properties = get_media_properties()
    if out_file is None:
        out_file = mediatools.utilities.add_postfix(video_file, "deshake_%dx%d" % (width,height))
...
def crop(video_file, width, height, top, left, out_file = None):
    ...
    properties = get_media_properties()
    if out_file is None:
        out_file = mediatools.utilities.add_postfix(video_file, "crop_%dx%d-%dx%d" % (width,height, top, left))
...
def probe_file(file):
    ''' Returns file probe (media specs) '''
    if mediatools.utilities.is_media_file(file):
        try:
            properties = get_media_properties()
...
def get_file_specs(file):
    ...
    #if file_type == 'image2':
    specs['type'] = mediatools.utilities.get_file_type(file)
    ...
    elif mediatools.utilities.is_video_file(file):
        specs['format'] = mediatools.utilities.get_file_extension(file)
...
def get_mp3_tags(file):
    from mp3_tagger import MP3File
    if mediatools.utilities.get_file_extension(file).lower() is not 'mp3':
        raise FileTypeError('File %s is not an mp3 file')
    # Create MP3File instance.
    mp3 = MP3File(file)
