...
import json
import shutil
from mediatools.utilities import *
...
class MediaFile:
    def __init__(self, filename):
        ...
        self.type = get_file_type(filename)
    ...
    def get_specs(self):
        if self.specs is None:
            ...
            debug(1, json.dumps(self.specs, sort_keys=True, indent=3, separators=(',', ': ')))
...
class AudioFile(MediaFile):
    ...
    def get_tags(self):
        ...
        if get_file_extension(self.filename).lower() is not 'mp3':
...
def build_target_file(source_file, profile, properties):
    ...
    return add_postfix(source_file, profile, extension)
...
def encode(source_file, target_file, profile, **kwargs):
    ...
    debug(2, ffmpeg.get_args(stream))
    debug(1, "%s --> %s" % (source_file, target_file))
    try:
...
def encode_album_art(source_file, album_art_file, **kwargs):
    ...
    properties = get_media_properties()
    target_file = add_postfix(source_file, 'album_art')
...
def rescale(image_file, width, height, out_file = None):
    properties = get_media_properties()
    if out_file is None:
        out_file = add_postfix(image_file, "%dx%d" % (width,height))
...
def deshake(video_file, width, height, out_file = None):
    ...
    properties = get_media_properties()
    if out_file is None:
        out_file = add_postfix(video_file, "deshake_%dx%d" % (width,height))
...
def crop(video_file, width, height, top, left, out_file = None):
    ...
    properties = get_media_properties()
    if out_file is None:
        out_file = add_postfix(video_file, "crop_%dx%d-%dx%d" % (width,height, top, left))
