...
{
	print "
$me -i <inputfile> [-o <outputfile>] [-s <starttime>] [-e <endtime>] [-h] [-g] [-r]\n\
...
my @starttab = split(/\s*,\s*/, $starttime);
my @endtab = split(/\s*,\s*/, $endtime);
die("Nbr of starttimes and endtimes ($#starttab and $#endtab) must be the same, aborting. Type $me -h for help\n") if ($#starttab != $#endtab);

my $profile = 'direct';
my @filelist;

for ($i=0; $i <= $#starttab; $i++) {
	...
	Trace::trace (1,"Cutting video file $ifile  between \"$starttab[$i]\" and \"$endttab[$i]\", output into \"$ofile\"\n");
